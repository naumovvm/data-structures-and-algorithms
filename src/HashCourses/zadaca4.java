...
//            Sara Dobreva with salary 2700 from address 10.73.60.29 who spent 2500
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            String[] splitter = ip_address.split("\\.");
            String networkAddress = splitter[0] + "." + splitter[1] + "." + splitter[2];
            Person person = new Person(name, lastName, salary, networkAddress, time, city, price);

            if (convertedTime > 1159) {
                SLLNode<MapEntry<String, Person>> node = hash.search(networkAddress);
                if(node==null){
                    hash.insert(networkAddress, person);
                }else{
                    if(convertedTime<Integer.parseInt(node.element.value.time.replace(":", ""))){
                        hash.insert(networkAddress, person);
                    }else{
        ...
        for (int i = 0; i < m; i++) {
            ...
            SLLNode<MapEntry<String, Person>> node = hash.search(networkAddress);
            SLLNode<MapEntry<String, Person>> nodeCounter = hash.search(networkAddress);
