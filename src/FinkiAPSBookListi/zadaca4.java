...
public class zadaca4 {
    ...
    public static void main(String[] args) { // sortiranje listi
        ...
        while (node1 != null) {
            spoenaLista.insertLast(node1.element);
            node1 = node1.succ;
        }

        while (node2 != null) {
            spoenaLista.insertLast(node2.element);
            node2 = node2.succ;
        }

        SLLNode<Integer> node;
        SLLNode<Integer> nodeToCompare;
        int length = spoenaLista.size();
        for (int i = 0; i < length; i++) {
            node = spoenaLista.getFirst();
            nodeToCompare = node.succ;

            while (nodeToCompare != null) {
                if (node.element > nodeToCompare.element) {
                    Integer temp = node.element;
                    node.element = nodeToCompare.element;
                    nodeToCompare.element = temp;
                }
                node = nodeToCompare;
        ...
        System.out.println(spoenaLista);
