...
public class zadaca12 { // Pocetok na obht, 226 strana, Crven krst
    ...
    public static void main(String[] args) {
        ...
        int n = input.nextInt();
        input.nextLine();
        OBHT<String, Integer> statistika = new OBHT<>(2 * n + 1);

        for (int i = 0; i < n; i++) {
            String[] vlez = input.nextLine().split(" ");
            String krvnaGr = vlez[1];

            if(krvnaGr.contains("+") || krvnaGr.contains("-")){
                krvnaGr = krvnaGr.replaceAll("[123456789]", "");
            }

            int node = statistika.search(krvnaGr);

            if (node == -1) {
                statistika.insert(krvnaGr, 1);
            } else {
                statistika.insert(krvnaGr, statistika.buckets[node].value + 1);
            }
        }
