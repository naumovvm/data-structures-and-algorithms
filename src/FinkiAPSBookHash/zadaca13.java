...
public class zadaca13 { // Kvalitet na vozduh, str 230
    ...
    public static void main(String[] args) {
        ...
        int n = input.nextInt();
        input.nextLine();
        OBHT<String, ArrayList<Double>> cesticki = new OBHT<>(2 * n + 1);

        for (int i = 0; i < n; i++) {
            String[] vlez = input.nextLine().split(" ");
            String naselba = vlez[0];
            Double merka = Double.parseDouble(vlez[1]);

            int search = cesticki.search(naselba);
            if (search == -1) {
                ArrayList<Double> merki = new ArrayList<>();
                merki.add(merka);
                cesticki.insert(naselba, merki);
            } else {
                ArrayList<Double> merki = cesticki.buckets[search].value;
                merki.add(merka);
                cesticki.insert(naselba, merki);
            }
        }
        ...
        if (search == -1) {
            System.out.println("Nema info");
        } else {
            ArrayList<Double> merki = cesticki.buckets[search].value;
            Double rezultat = 0.0;
            for (Double merka : merki) {
                rezultat += merka;
            }
            rezultat /= merki.size();
            BigDecimal rez = new BigDecimal(rezultat).setScale(2, BigDecimal.ROUND_HALF_UP);
